...
#import model and froze top layer
...
model_resnet50 = Sequential([
    ...
    Dense(246, activation = 'relu',kernel_regularizer=l2(0.01)),
    Dropout(0.9),
...
train_resnet = model_resnet50.fit(X_train, y_train_encoded, epochs = 9, batch_size = 36, validation_data = (X_test, y_test_encoded))

resnet_loss, test_resnet = model_resnet50.evaluate(X_test, y_test_encoded)

print(f'ResNet50 Accuracy: {test_resnet}')

#show result
plt.plot(train_resnet.history['accuracy'], label = 'Training Accuracy')
plt.plot(train_resnet.history['val_accuracy'], label = 'Testing Accuracy')
...
plt.plot(train_resnet.history['loss'], label  = 'Training Loss')
plt.plot(train_resnet.history['val_loss'], label = 'Testing Loss')
...
#show example images of model prediction vs true
import random
...
sample_predictions = model_resnet50.predict(sample_images)
...
plot_model(model_resnet50, to_file='model_architecture.png', show_shapes=True)
